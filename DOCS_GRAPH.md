...
- **AGENTS.md (2026-09-02):** each repo gets `AGENTS.md` as a symlink to `CLAUDE.md`, so Codex / ChatGPT reads the same control doc Claude does. Symlink, not rename — the 6-document standard keeps naming CLAUDE.md. Rolled out 2026-09-02 to every repo in the ecosystem — merged in jh-api-dev (#342) and 108heros-flutter-dev (#202), open in the other 16 mirrors and here; `pos108-admin-dev` left as is (it already has a real `AGENTS.md` with `CLAUDE.md` as the pointer — the inverse, already cross-tool). Each PR also records the mechanism in the repo's own `CLAUDE.md`.
